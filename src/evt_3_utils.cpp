  ...
  uint32_t * last_evt_stamp)
{
  ...
  for (size_t i = 0; i < num_bytes;
       i += bytes_per_event, p += bytes_per_event) {
    uint16_t ex;
    uint16_t ey;
    uint64_t etu;
    ...
    for (size_t i = 0; i < numRead; i++) {
      switch (buffer[i].code) {
        case Code::ADDR_X: {
          ...
          const uint64_t ts_ros =
            updater->getROSTime() + (ts - t0_sensor) * 1000LL;
        ...
        case Code::OTHERS: {
          ...
          if (subtype != SubType::MASTER_END_OF_FRAME) {
            std::cout << "ignoring OTHERS code: " << toString(subtype)
                      << std::endl;
          }
        } break;
