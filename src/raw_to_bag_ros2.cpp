...
class MessageUpdaterROS2 : public evt_3_utils::MessageUpdater
{
public:
  explicit MessageUpdaterROS2(
  ...
  ~MessageUpdaterROS2() { writer_.reset(); }
    ...
    uint64_t ts_ros, uint16_t ex, uint16_t ey, uint8_t polarity) override
  {
    ...
    event_array_msgs::mono::encode(
      reinterpret_cast<uint64_t *>(buffer), static_cast<bool>(polarity), ex, ey,
      dt);
    ...
    if (msg_.events.size() >= MAX_MESSAGE_SIZE || dt > MAX_MESSAGE_TIME) {
      ...
      writer_->write(
        serialized_msg, topic_, "event_array_msgs/msg/EventArray",
...
int main(int argc, char ** argv)
{
  ...
  metavision_ros_tools::MessageUpdaterROS2 updater(
